...
Then point your clients' DNS at `LANCACHE_IP`. Nothing else on the client changes.
...
- cachic deliberately does not do DNS interception (PRD N1).

`cache` is cachic. It resolves upstream hostnames through `UPSTREAM_DNS`, never the system
resolver, so it cannot be tricked into resolving a CDN hostname through the DNS server that is
pointing at itself. That is a structural guarantee, not a configuration you can get wrong: the
...
```sh
curl -H 'Host: lancache.steamcontent.com' http://LANCACHE_IP/lancache-heartbeat -i
```
